...
public class LibraryWSImpl implements LibraryWS {
    ...
    @Override
    @RequestMapping(value="/login/bookId", method = RequestMethod.GET)
    public ResponseEntity getBookByIdService(@RequestParam("bookId") Long bookId){

        try{
            BookModel book = libraryService.getBookById(bookId);
            log.info("Retrieved book by name in getBookById()");
            return new ResponseEntity<>(book, HttpStatus.OK);
        } catch(Exception e){
            log.info("Retrieval Exception in getBookById()");
        }
        log.info("Failed to Retrieve in getBookById()");
